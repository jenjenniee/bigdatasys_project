...
def index():
  ...
  {
    "$group": {
      _id: { Cuisine: "$Cuisine", OrderWeek: "$OrderWeek" },
      total_orders: { "$sum": 1 }
    }
  },
  {
    "$group": {
      _id: "$_id.OrderWeek",
      top_cuisine: { "$push": { Cuisine: "$_id.Cuisine", total_orders: "$total_orders" } }
    }
  },
  {
    "$project": {
      _id: 0,
      OrderWeek: "$_id",
      top_cuisine: { "$slice": ["$top_cuisine", 5] }
    }
  }
])
  ...
  {
    "$group": {
      _id: { month: { "$month": { "$dateFromString": { dateString: "$Date" } } }, province: "$AREA.Province" },
      total_orders: { "$sum": 1 }
    }
  },
  {
  ...
  },
  {
    "$group": {
      _id: "$_id.month",
      top_regions: { "$push": { province: "$_id.province", total_orders: "$total_orders" } }
    }
  },
  {
    "$project": {
      _id: 0,
      month: "$_id",
      top_regions: { "$slice": ["$top_regions", 5] }
    }
  }
])
